...
WINDOW_SIZE = 600 
GRID_LINE_WIDTH = 2 
SYMBOL_WIDTH = WINDOW_SIZE/20 # pixels - adjust ratio
...
class Game(Tk):
    ...
    # placeholder title screen
        ...
        self.canvas.create_text(
			WINDOW_SIZE/2,
			WINDOW_SIZE/3,
			text='TIC TAC TOE', fill='blue4',
			font=('Battlestar', int(-WINDOW_SIZE/12), 'bold'))

        self.canvas.create_text(
			int(WINDOW_SIZE/2),
			int(WINDOW_SIZE/2.5),
			text='[play]', fill='white',
			font=('Times New Roman', int(-WINDOW_SIZE/25), 'bold'))

        self.canvas.create_text(
			int(WINDOW_SIZE/2),
			int(WINDOW_SIZE/2),
			text='Project made by : ', fill='light sky blue',
			font=('Britannic Bold', int(-WINDOW_SIZE/20), 'italic', 'underline'))

        self.canvas.create_text(
			int(WINDOW_SIZE/2),
			int(WINDOW_SIZE/1.8),
			text='1.BHUPESH PATIL', fill='blue2',
			font=('Showcard Gothic', int(-WINDOW_SIZE/20)))

        self.canvas.create_text(
			int(WINDOW_SIZE/2),
			int(WINDOW_SIZE/1.6),
			text='2.ADITYA JAMBHALE', fill='blue2',
			font=('Showcard Gothic', int(-WINDOW_SIZE/20)))

        self.canvas.create_text(
			int(WINDOW_SIZE/2),
			int(WINDOW_SIZE/1.4),
			text='3.AKSHAY NAIKWADI', fill='blue2',
			font=('Showcard Gothic', int(-WINDOW_SIZE/20)))
